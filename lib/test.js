...
module.exports = {
	...
	tests: [
		...
		function testRefreshStaleLocalServerIp(test) {
			// Stored IP from a previous run can drift (Wi-Fi/VPN/Docker bridge).
			// ensureLocalServerRegistration / refreshLocalServerIp must rewrite it.
			var bogus_ip = '203.0.113.50';
			storage.listFindUpdate('global/servers', { hostname: server.hostname }, { ip: bogus_ip }, function(err) {
				test.ok( !err, "No error setting stale IP" );

				cronicle.refreshLocalServerIp(function(err) {
					test.ok( !err, "No error refreshing local server IP" );

					storage.listFind('global/servers', { hostname: server.hostname }, function(err, item) {
						test.ok( !err, "No error loading server entry" );
						test.ok( !!item, "Server entry exists" );
						test.ok( item.ip == server.ip, "Stored IP refreshed to current server IP" );
						test.ok( item.ip != bogus_ip, "Bogus IP was replaced" );
						test.done();
					});
				});
			});
		...
		function testEnsureRegistrationWhenMissingFromServers(test) {
			// Forced manager path: host missing from global/servers must be re-added
			// and become manager-eligible again (simulates Docker HOSTNAME change after
			// the old entry was removed, or first boot of a forced manager against existing data).
			storage.listFindDelete('global/servers', { hostname: server.hostname }, function(err) {
				test.ok( !err, "No error removing local server from cluster list" );

				// Simulate pre-manager eligibility failure state
				cronicle.multi.eligible = false;
				cronicle.multi.cluster = false;
				cronicle._startupServers = [];

				cronicle.ensureLocalServerRegistration({ forceManagerGroup: true }, function(err) {
					test.ok( !err, "No error ensuring local registration" );
					test.ok( cronicle.multi.eligible == true, "Server is eligible after ensure" );
					test.ok( cronicle.multi.cluster == true, "Server is in cluster after ensure" );

					storage.listFind('global/servers', { hostname: server.hostname }, function(err, item) {
						test.ok( !err, "No error loading re-added server" );
						test.ok( !!item, "Server was re-added to global/servers" );
						test.ok( item.hostname == server.hostname, "Hostname matches" );
						test.ok( item.ip == server.ip, "IP matches current server IP" );
						test.done();
					});
				});
			});
		...
		function testSingleNodeHostnameDriftSelfHeal(test) {
			// Classic single-node setup locked maingrp + servers to an old hostname.
			// replaceSingleServer should swap the entry and rewrite maingrp.
			var old_hostname = 'old-cronicle-host.example';
			var old_ip = '198.51.100.10';

			async.series([
				function(next) {
					storage.listFindUpdate('global/servers', { hostname: server.hostname }, {
						hostname: old_hostname,
						ip: old_ip
					}, next);
				},
				function(next) {
					storage.listFindUpdate('global/server_groups', { id: 'maingrp' }, {
						regexp: '^(' + Tools.escapeRegExp(old_hostname) + ')$'
					}, next);
				},
				function(next) {
					// Simulate idle restart state (not yet manager/worker) with stale identity
					cronicle.multi.eligible = false;
					cronicle.multi.cluster = false;
					cronicle.multi.worker = false;
					cronicle.multi.manager = false;
					cronicle._startupServers = [{ hostname: old_hostname, ip: old_ip }];
					test.ok( cronicle.shouldSelfHealAsManager() == true, "Single-node drift detected" );

					cronicle.ensureLocalServerRegistration({
						forceManagerGroup: true,
						replaceSingleServer: true
					}, next);
				},
				function(next) {
					storage.listFind('global/servers', { hostname: server.hostname }, function(err, item) {
						test.ok( !err, "No error finding healed server entry" );
						test.ok( !!item, "Current hostname is in servers list" );
						test.ok( item.ip == server.ip, "Healed entry has current IP" );
						next(err);
					});
				},
				function(next) {
					storage.listFind('global/servers', { hostname: old_hostname }, function(err, item) {
						// listFind returns err when not found
						test.ok( !item, "Stale hostname was replaced (not left as orphan)" );
						next();
					});
				},
				function(next) {
					storage.listFind('global/server_groups', { id: 'maingrp' }, function(err, group) {
						test.ok( !err && !!group, "maingrp exists" );
						var re = null;
						try { re = new RegExp(group.regexp); } catch (e) {;}
						test.ok( !!re && !!server.hostname.match(re), "maingrp matches current hostname" );
						test.ok( !old_hostname.match(re) || group.regexp.indexOf(server.hostname) > -1, "maingrp updated away from only-old-host lock" );
						next();
					});
				}
			], function(err) {
				test.ok( !err, "No error during single-node heal: " + (err || '') );
				test.ok( cronicle.multi.eligible == true, "Eligible after heal" );
				// restore manager flags for remaining API tests (becameManager already set earlier)
				cronicle.multi.manager = true;
				cronicle.multi.worker = false;
				cronicle.multi.cluster = true;
				cronicle.multi.eligible = true;
				cronicle.multi.managerHostname = server.hostname;
				test.done();
			});
		},

		function testShouldNotSelfHealMultiServerCluster(test) {
			// If multiple servers are registered and we are not one of them, do not hijack manager.
			cronicle.multi.eligible = false;
			cronicle.multi.cluster = false;
			cronicle.multi.worker = false;
			cronicle.multi.manager = false;
			cronicle._startupServers = [
				{ hostname: 'node-a.example', ip: '192.0.2.1' },
				{ hostname: 'node-b.example', ip: '192.0.2.2' }
			];
			test.ok( cronicle.shouldSelfHealAsManager() == false, "Multi-server cluster is not self-healed" );

			// restore manager flags for remaining API tests
			cronicle.multi.manager = true;
			cronicle.multi.worker = false;
			cronicle.multi.cluster = true;
			cronicle.multi.eligible = true;
