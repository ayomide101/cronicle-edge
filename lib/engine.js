...
module.exports = Class.create({
	...
	startup: function (callback) {
		...
		// save default api key before clearing env
		this._defaultApiKey = process.env["CRONICLE_api_key"] || '';
		...
		delete process.env["CRONICLE_Storage__SQL__connection__password"];
		delete process.env["CRONICLE_api_key"];
		...
		// determine manager server eligibility and become manager when appropriate
		this.checkmanagerEligibility(function () {
			...
			if (self.server.config.get('manager')) {
				// Forced manager mode must always win, even if storage has a stale hostname/IP
				// from a previous run (Docker HOSTNAME change, network IP drift, etc.).
				self.ensureLocalServerRegistration({
					forceManagerGroup: true,
					replaceSingleServer: self.shouldSelfHealAsManager()
				}, function (err) {
					if (err) self.logError('manager', "Failed to ensure local manager registration: " + err);
					self.multi.eligible = true;
					self.gomanager();
					finishStartup();
				});
			}
			else if (self.multi.eligible) {
				// Refresh our stored IP if it changed since last run
				self.refreshLocalServerIp(function () {
					finishStartup();
				});
			}
			else if (self.shouldSelfHealAsManager()) {
				// Single-node install whose stored identity no longer matches this host:
				// re-register and become manager immediately so the UI is not stuck waiting.
				self.logDebug(3, "Single-node identity drift detected -- re-registering as manager");
				self.ensureLocalServerRegistration({ forceManagerGroup: true, replaceSingleServer: true }, function (err) {
					if (err) {
						self.logError('manager', "Failed to self-heal manager registration: " + err);
	...
	checkmanagerEligibility: function (callback) {
		...
		this.storage.listGet('global/servers', 0, 0, function (err, servers) {
			...
			// stash for startup self-heal decisions
			self._startupServers = servers || [];
	...
	shouldSelfHealAsManager: function () {
		// True when this looks like a solo install whose stored manager identity no longer
		// matches the current host (hostname change, new container id, etc.).
		// Never hijack a multi-server cluster, and never treat an empty servers list as
		// drift — that usually means setup has not run yet (or a worker without storage).
		if (this.multi.worker || this.multi.manager || this.multi.cluster) return false;
		var servers = this._startupServers || [];
		if (servers.length === 1 && servers[0].hostname !== this.server.hostname) return true;
		return false;
	},

	refreshLocalServerIp: function (callback) {
		// Update stored IP for this hostname when it has changed (network/interface drift)
		const self = this;
		var hostname = this.server.hostname;
		var ip = this.server.ip || '';

		this.storage.listFind('global/servers', { hostname: hostname }, function (err, item) {
			if (!item) {
				if (callback) callback();
				return;
			}
			...
			self.logDebug(4, "Refreshing stored IP for local server", { hostname: hostname, old_ip: item.ip, ip: ip });
			self.storage.listFindUpdate('global/servers', { hostname: hostname }, { ip: ip }, function (err) {
				if (err) self.logError('manager', "Failed to refresh local server IP: " + err);
				if (callback) callback(err);
			});
		});
	},

	ensureLocalServerRegistration: function (opts, callback) {
		// Ensure this host is present in global/servers with the current IP, and that a
		// manager server group matches us when we need to become (or stay) manager.
		// opts.forceManagerGroup: rewrite/expand manager group so we are eligible
		// opts.replaceSingleServer: when exactly one stale server entry exists, replace it
		const self = this;
		...
		var hostname = this.server.hostname;
		var ip = this.server.ip || '';
		var stub = { hostname: hostname, ip: ip };

		async.series([
			function (next) {
				self.storage.listGet('global/servers', 0, 0, function (err, servers) {
					if (err) servers = [];
					servers = servers || [];

					var existing = Tools.findObject(servers, { hostname: hostname });
					if (existing) {
						if (existing.ip !== ip) {
							self.logDebug(4, "Updating local server IP in cluster list", { hostname: hostname, old_ip: existing.ip, ip: ip });
							return self.storage.listFindUpdate('global/servers', { hostname: hostname }, { ip: ip }, next);
						}
						return next();
					}

					// Hostname not in list
					if (opts.replaceSingleServer && servers.length === 1) {
						var stale = servers[0];
						self.logDebug(3, "Replacing stale single-node server entry with current host", {
							old_hostname: stale.hostname,
							old_ip: stale.ip,
							hostname: hostname,
							ip: ip
						});
						return self.storage.listFindUpdate('global/servers', { hostname: stale.hostname }, stub, next);
					}

					self.logDebug(3, "Adding local server to cluster list", stub);
					self.storage.listPush('global/servers', stub, next);
				});
			},
			function (next) {
				if (!opts.forceManagerGroup) return next();

				self.storage.listGet('global/server_groups', 0, 0, function (err, groups) {
					if (err) groups = [];
					groups = groups || [];

					// Already matched by a manager group?
					for (var idx = 0, len = groups.length; idx < len; idx++) {
						var group = groups[idx];
						if (!group.manager || !group.regexp) continue;
						var regexp = null;
						try { regexp = new RegExp(group.regexp); }
						catch (e) { regexp = null; }
						if (regexp && hostname.match(regexp)) {
							self.logDebug(6, "Local host already matches manager group: " + group.title);
							return next();
						}
					}

					var newRegexp = '^(' + Tools.escapeRegExp(hostname) + ')$';
					var maingrp = Tools.findObject(groups, { id: 'maingrp' });

					if (maingrp) {
						// Single-host lock or single-node heal: replace maingrp regexp (same as storage-cli reset).
						// Otherwise expand so multi-manager configs are not wiped.
						var replace = !!opts.replaceSingleServer;
						if (!replace && maingrp.regexp) {
							// setup-style exact host lock: ^(host)$ or ^host$
							if (/^\^\(?[^|]+?\)?\$$/.test(maingrp.regexp)) replace = true;
						}
						if (!replace && groups.filter(function (g) { return g.manager; }).length <= 1) {
							// only one manager group and we don't match it -- take it over for solo installs
							replace = true;
						}

						var update = { regexp: newRegexp };
						if (!replace && maingrp.regexp) {
							// expand: ^(old)$ + new host -> ^(old|new)$
							var body = String(maingrp.regexp).replace(/^\^/, '').replace(/\$$/, '');
							if (body.charAt(0) === '(' && body.charAt(body.length - 1) === ')') {
								body = body.substring(1, body.length - 1);
							}
							update.regexp = '^(' + body + '|' + Tools.escapeRegExp(hostname) + ')$';
						}

						self.logDebug(3, "Updating manager group so local host is eligible", {
							id: maingrp.id,
							old_regexp: maingrp.regexp,
							regexp: update.regexp
						});
						return self.storage.listFindUpdate('global/server_groups', { id: 'maingrp' }, update, next);
					}

					// No maingrp -- try first manager group, or create maingrp
					var firstManager = null;
					for (var i = 0; i < groups.length; i++) {
						if (groups[i].manager) { firstManager = groups[i]; break; }
					}
					if (firstManager) {
						self.logDebug(3, "Expanding manager group for local host", { id: firstManager.id });
						var body2 = String(firstManager.regexp || '').replace(/^\^/, '').replace(/\$$/, '');
						if (body2.charAt(0) === '(' && body2.charAt(body2.length - 1) === ')') {
							body2 = body2.substring(1, body2.length - 1);
						}
						var expanded = body2
							? '^(' + body2 + '|' + Tools.escapeRegExp(hostname) + ')$'
							: newRegexp;
						return self.storage.listFindUpdate('global/server_groups', { id: firstManager.id }, { regexp: expanded }, next);
					}

					self.logDebug(3, "Creating maingrp manager group for local host");
					self.storage.listPush('global/server_groups', {
						id: 'maingrp',
						title: 'Manager Group',
						regexp: newRegexp,
						manager: 1
			...
			function (next) {
				self.multi.cluster = true;
				self.checkmanagerEligibility(function () {
					// Forced registration should leave us eligible; ensure that even if groups were odd
					if (opts.forceManagerGroup) self.multi.eligible = true;
					next();
				});
			}
		], function (err) {
	...
	gomanager: function () {
		...
		if (this.becameManager) {
			const err_msg = "MANAGER CONFLICT: This server is becoming manager twice.  Shutting down immediately.  Please file a GitHub issue and include this full log entry.";
		...
		// Keep stored identity current so restarts / IP-based clients see the live address
		this.refreshLocalServerIp(function () {});
		...
		// provision default API key from CRONICLE_api_key env variable if set
		if (this._defaultApiKey) {
			var defaultApiKey = this._defaultApiKey;
			this.storage.listFind('global/api_keys', { key: defaultApiKey }, function(err, item) {
				if (!item) {
					var record = {
						id: self.getUniqueID('k'),
						title: 'Default',
						key: defaultApiKey,
						active: 1,
						username: 'admin',
					...
					self.storage.listUnshift('global/api_keys', record, function(err) {
						if (err) self.logError('api_key', "Failed to create default API key: " + err);
						else self.logDebug(3, "Default API key provisioned from environment");
					});
				}
			});
		}
	...
	goworker: function () {
		...
		if (this.becameManager) {
			const err_msg = "MANAGER CONFLICT: This server is being forced to demote itself from manager primary to worker.  Shutting down immediately.  Please file a GitHub issue and include this full log entry.";
			const stack = (new Error()).stack;
			this.logDebug(1, err_msg, { stack });
			this.logError('multi', err_msg, { stack });
			this.server.shutdown();
