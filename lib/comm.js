...
module.exports = Class.create({
	...
	addServer: function(server, args) {
		...
		// Never dial ourselves. Hostname is the normal case; also skip when a stale
		// cluster entry points at our current IP under a different hostname (identity drift).
		if (server.hostname === this.server.hostname) return;
		if (server.ip && this.server.ip && server.ip === this.server.ip) {
			this.logDebug(5, "Skipping worker add for stale same-IP alias: " + server.hostname + " (" + server.ip + ")");
			return;
		}
